...
[![Build Status](https://img.shields.io/github/checks-status/mpapenbr/iracelog-cli/main?color=black&style=for-the-badge&logo=github)][github-actions]
...
```sh
git clone https://github.com/mpapenbr/iracelog-cli
cd iracelog-cli
```
...
After cloning the repository you already changed into the project dir iracelog-cli.
...
This section will guide you to setup a fully-functional local copy of `iracelog-cli`
...
from [`iracelog-cli's` Makefile][makefile-file] (most of which are
OS-independent)!

### Installing dependencies

To install all dependencies associated with `iracelog-cli`, run the
command
...
Tests in `iracelog-cli` are classified as _fast_ and _slow_ - depending
...
`iracelog-cli` is powered by a template generated using [`cookiecutter-go`][cookiecutter-go-link] which is based on [`go-template`][go-template-link]
