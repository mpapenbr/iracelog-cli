...
.PHONY: install
## `install`: Fetch dependencies needed to run `iracelog-cli`
install:
...
.PHONY: run
## :
## `run`: Run `iracelog-cli` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `iracelog-cli` in debug mode
